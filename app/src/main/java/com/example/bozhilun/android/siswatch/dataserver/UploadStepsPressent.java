...
public class UploadStepsPressent {
    ...
    public void pressUploadStepData(Context mContext,String url,String jsonStr){
        if(uploadStepsView != null){
            uploadStepsModel.modelUpdateStr(mContext,url,jsonStr, new SubscriberOnNextListener<String>() {
                @Override
                public void onNext(String s) {
                    Log.e("ppp","---s="+s);
                    uploadStepsView.uploadResultData(s);
