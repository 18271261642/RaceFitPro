...
public class FrendHeartActivity extends WatchBaseActivity implements RequestView {
    ...
    int countNumber = 0;
    private List<Integer> tmpAvgList = new ArrayList<>();
...
//        commentB30ShareImg.setVisibility(View.VISIBLE);
            ...
            try {
                ...
                if (jsonObject.getInt("code") == 200) {
                    ...
                    List<FrendHaretBean.FriendHeartRateBean> tmpList = new Gson().fromJson(data,
                            new TypeToken<List<FrendHaretBean.FriendHeartRateBean>>() {
                            }.getType());
                    if (tmpList != null)
    ...
    public void failedData(int what, Throwable e) {
        closeLoadingDialog();
        showChartAndList(new ArrayList<FrendHaretBean.FriendHeartRateBean>());
    }
    ...
    void showChartAndList(List<FrendHaretBean.FriendHeartRateBean> heartList) {
        ...
        for (FrendHaretBean.FriendHeartRateBean item :
            ...
            if (item.getHeartRate() > 0) {
                dataList.add(item);
                int heartValue = item.getHeartRate();
                tmpAvgList.add(heartValue);
                countNumber += heartValue;
            }
        }
        ...
        String maxValue = Collections.max(allheartList)+"";
        String minValue = Collections.min(tmpAvgList)+"";
        String avgValue = (countNumber / dataList.size())+"";
        showAvgValue(maxValue,minValue,avgValue);

    }

    private void showAvgValue(String maxV,String minV,String avgV){
        detailHeartMaxTv.setText(maxV);
        detailHeartMinTv.setText(minV);
        detailHeartAvgTv.setText(avgV);
    }
